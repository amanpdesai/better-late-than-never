...
def summarise_image(label: str, payload: Optional[Dict[str, str]]) -> Iterable[str]:
    ...
    # Filter out problematic content that might trigger safety filters
    problematic_words = [
        "self-harm", "self-deprecating", "mental exhaustion", "dark coping", 
        "existential dread", "violent", "threat", "suicide", "depression",
        "self-harm", "self-deprecating", "mental exhaustion", "dark coping"
    ]
    ...
    def filter_content(text: str) -> str:
        ...
        for word in problematic_words:
            if word in text.lower():
                text = text.replace(word, "relatable").replace(word.title(), "Relatable")
        return text
    
    description = filter_content(payload.get("description") or payload.get("topic") or "")
    humor = filter_content(payload.get("humor") or "")
    template = filter_content(payload.get("meme_template") or "")
